...
pub enum GovernanceDataKey {
    ...
    // === إضافات انتخاب أعضاء خزينة DAO ===
    DAOMember(Address),
    DAOMemberVote(Address, Address),  // (voter, candidate)
    DAOElectionStart,
    DAOElectionEnd,
    DAOElectionActive,
}
...
// === نسب توزيع المصادرات الجديدة: 50/40/10 ===
const FORFEITURE_PLATFORM_SHARE: i128 = 50;   // 50% لخزينة المنصة
const FORFEITURE_SAFETY_SHARE: i128 = 40;     // 40% لصندوق الأمان
const FORFEITURE_DAO_SHARE: i128 = 10;         // 10% لخزينة DAO

// === ثوابت انتخاب أعضاء خزينة DAO ===
const DAO_MEMBER_COUNT: u32 = 5;               // 5 أعضاء
const DAO_ELECTION_DURATION: u64 = 604_800;     // 7 أيام للانتخابات
const DAO_TERM_DURATION: u64 = 15_768_000;      // 6 أشهر (182.5 يوم)
const DAO_MIN_VOTES_TO_WIN: i128 = 10;          // الحد الأدنى للأصوات للفوز
const DAO_MULTISIG_THRESHOLD: u32 = 3;          // 3 من 5 توقيعات مطلوبة للصرف
...
impl MithaqGovernance {
    ...
    pub fn execute_proposal(
        ...
        if approval_percentage >= APPROVAL_THRESHOLD && total_votes > 0 {
            // ✅ الاقتراح مقبول – إعادة 10 Azman لمقدم الاقتراح
    ...
    /// بدء انتخابات أعضاء خزينة DAO (للمسؤول فقط)
    pub fn start_dao_election(env: Env, admin: Address) {
        ...
        DAOElectionStarted { start_time: now, end_time }.publish(&env);
    }

    /// التصويت لمرشح لعضوية خزينة DAO
    pub fn vote_for_dao_member(env: Env, voter: Address, candidate: Address) {
        ...
        // منع التصويت المزدوج لنفس المرشح
    ...
    /// إنهاء الانتخابات واختيار أفضل 5 مرشحين
    pub fn finalize_dao_election(env: Env, admin: Address) {
        ...
        // في بيئة الإنتاج، سيتم جمع جميع المرشحين وأصواتهم واختيار أفضل 5
        // هنا نقدم الهيكل الأساسي القابل للتوسع

        env.storage().persistent().set(&GovernanceDataKey::DAOElectionActive, &false);

        // تنظيف مفاتيح التصويت القديمة
        let start_time: u64 = env.storage().persistent()
            .get(&GovernanceDataKey::DAOElectionStart)
            .unwrap_or(0);

        // نشر حدث إتمام الانتخابات
        let term_expires = now + DAO_TERM_DURATION;
        // في الإنتاج: يتم نشر حدث لكل عضو منتخب
    }

    /// صرف من خزينة DAO (يتطلب 3 من 5 توقيعات)
    pub fn dao_treasury_disburse(
        ...
        // التحقق من أن الموقعين هم أعضاء DAO حالياً
        // التحقق من 3 توقيعات على الأقل
        if signatures.len() < DAO_MULTISIG_THRESHOLD {
            panic!("Insufficient signatures. Required: 3 of 5");
        }

        // التحقق من صحة كل توقيع
        for signer in signatures.iter() {
            signer.require_auth();
            // التحقق من أن الموقع عضو حالي في DAO
    ...
    /// الحصول على عدد أعضاء DAO الحاليين
    pub fn get_dao_member_count(env: Env) -> u32 {
        // في الإنتاج: عد الأعضاء النشطين
        DAO_MEMBER_COUNT
    }

    /// الحصول على حد التوقيعات المتعددة
    ...
    pub fn update_azman_token(env: Env, admin: Address, new_token: Address) {
        let stored_admin = get_admin(&env);
        if admin != stored_admin {
            panic!("Unauthorized: Only Admin");
        }
    ...
    pub fn update_platform_treasury(env: Env, admin: Address, new_treasury: Address) {
        let stored_admin = get_admin(&env);
        if admin != stored_admin {
            panic!("Unauthorized: Only Admin");
        }
    ...
    pub fn update_safety_fund(env: Env, admin: Address, new_fund: Address) {
        let stored_admin = get_admin(&env);
        if admin != stored_admin {
            panic!("Unauthorized: Only Admin");
        }
    ...
    pub fn update_dao_treasury(env: Env, admin: Address, new_treasury: Address) {
        let stored_admin = get_admin(&env);
        if admin != stored_admin {
            panic!("Unauthorized: Only Admin");
        }
