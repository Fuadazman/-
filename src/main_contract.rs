...
impl MithaqContract {
    ...
    pub fn release_milestone(env: Env, id: u64, caller: Address, milestone: u32) {
        ...
        // قراءة القيمة من extra_data كـ Val وتحويلها إلى u32
        let current_milestone: u32 = c.extra_data
            .get(String::from_str(&env, "milestone"))
            .unwrap_or(Val::from(0u32))
            .try_into_val(&env)
            .unwrap_or(0u32);

        if milestone <= current_milestone { panic!("Double spending/release attempt"); }

        // تخزين milestone الجديد كـ Val
        c.extra_data.set(String::from_str(&env, "milestone"), Val::from(milestone));
    ...
    pub fn pay_installment(env: Env, id: u64, caller: Address, installment_num: u32) {
        ...
        c.extra_data.set(String::from_str(&env, "installment"), Val::from(installment_num));
