...
pub struct Commitment {
    ...
    pub net_value: i128,
    pub down_payment: i128,    pub first_release_amount: i128,
...
pub struct CommitmentParams {
    pub original_value: i128,
    pub net_value: i128,
    pub down_payment: i128,    pub contract_type: Symbol,
    pub deadline: u64,
...
impl MithaqContract {
    ...
    fn get_liquidity_pool_state(env: &Env) -> LiquidityPoolState {
        ...
        let state: LiquidityPoolState = env.storage().persistent().get(&key).unwrap_or(LiquidityPoolState {
            total_pooled: 0,
            total_recycled_fees: 0,            active_contributors: 0,
            pool_utilization: 50,
    ...
    fn withdraw_from_pool(env: &Env, commitment_id: u64, contributed_amount: i128) {
        ...
        let total_escrow = Self::get_total_escrow_balance(env);        if total_escrow > 0 {
            pool.pool_utilization = (pool.total_pooled * 100) / total_escrow;
        } else {
    ...
    fn release_recycled_fees(env: &Env) -> i128 {
        ...
        if let Some((amount, recycle_end)) = env.storage().persistent().get::<_, (i128, u64)>(&recycle_key) {
            if env.ledger().timestamp() >= recycle_end {
                let mut pool = Self::get_liquidity_pool_state(env);                pool.total_recycled_fees = (pool.total_recycled_fees - amount).max(0);
    ...
    fn process_hybrid_liquidity(env: &Env, commitment: &Commitment, fees_paid: i128) {
        ...
        let new_utilization = if pool.total_pooled > 0 {
            let total_escrow = Self::get_total_escrow_balance(env);
            if total_escrow > 0 {                (pool.total_pooled * 100) / total_escrow
            } else {
                50
            }
        } else {
    ...
    pub fn create_commitment(
        ...
        let commitment = Commitment {
            ...
            platform_fee_percent: dyn_platform_fee, insurance_fee_percent: dyn_insurance_fee,            legal_doc_hash: params.legal_doc_hash.clone(),
    ...
    // ==================== 4. الإلغاء القانوني ====================    pub fn cancel_commitment(env: Env, id: u64, caller: Address, reason: Symbol) {
        caller.require_auth();
        let mut c = Self::get_commitment(&env, id);
    ...
    pub fn release_milestone(env: Env, id: u64, caller: Address, milestone: u32) {
        ...
        let amount_per_milestone = c.down_payment / 10;        let azman_addr = env.storage().persistent().get::<_, Address>(&DataKey::AzmanToken).unwrap();
    ...
    // ✅ تم تعديل الاسم إلى get_contract_admin لتجنب التضارب مع واجهة Soroban الافتراضية
