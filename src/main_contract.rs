...
use soroban_sdk::{
    ...
    Address, Env, String, Symbol, BytesN, Bytes, Map,
    token::{TokenClient}
};
...
pub struct Commitment {
    ...
    pub extra_data: Map<String, String>,
    pub created_at: u64,
...
// ===== 2. الاحداث =====
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractCreated { pub id: u64, pub creator: Address, pub contract_type: Symbol, pub net_value: i128, pub down_payment: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractAccepted { pub id: u64, pub counterparty: Address, pub escrowed_amount: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractCancelled { pub id: u64, pub by: Address, pub refunded: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct PaymentUpdated { pub id: u64, pub status: Symbol }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct DeliveryConfirmed { pub id: u64, pub amount: i128, pub proof_hash: String }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractCompleted { pub id: u64, pub final_payout: i128, pub fees_paid: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct AutoReleased { pub id: u64, pub payout: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct DisputeOpened { pub dispute_id: u64, pub contract_id: u64, pub by: Address, pub reason: String }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct DisputeResolved { pub dispute_id: u64, pub winner: Address, pub verdict: String, pub payout: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct MilestoneReleased { pub id: u64, pub milestone: u32, pub amount: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct InstallmentPaid { pub id: u64, pub installment: u32, pub amount: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ProposalRejected { pub proposal_id: String, pub developer: Address, pub fund_share: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractUpgraded { pub admin: Address, pub new_wasm_hash: BytesN<32> }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct LiquidityContributed { pub contract_id: u64, pub amount: i128, pub total_pooled: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct LiquidityWithdrawn { pub contract_id: u64, pub amount: i128, pub total_pooled: i128 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct FeesRecycled { pub amount: i128, pub recycle_end: u64 }
#[contractevent] #[derive(Clone, Debug, Eq, PartialEq)] pub struct PoolRebalanced { pub old_utilization: i128, pub new_utilization: i128, pub fee_adjustment: i128 }
...
// === ثوابت توزيع المصادرات (Forfeiture Split) ===
...
impl MithaqContract {
    ...
    fn recycle_fees(env: &Env, fee_amount: i128) {
        ...
        FeesRecycled { amount: fee_amount, recycle_end }.publish(env);
    }
    ...
    fn process_hybrid_liquidity(env: &Env, commitment: &Commitment, fees_paid: i128) {
        ...
        if old_utilization != new_utilization {
            ...
            PoolRebalanced { old_utilization, new_utilization, fee_adjustment }.publish(env);
        }
    }
    ...
    pub fn create_commitment(
        ...
        first_release_amount: i128,
        second_release_amount: i128,
        legal_doc_hash: String
    ) -> u64 {
        env.storage().persistent().get::<_, Address>(&DataKey::PiServer).expect("PiServer not set").require_auth();

        if net_value <= 0 { panic!("Net value must be > 0"); }
        if down_payment < 0 || down_payment > net_value { panic!("Invalid down payment amount"); }
        if creator == counterparty { panic!("Self-dealing is strictly prohibited"); }
        if first_release_amount + second_release_amount != net_value { panic!("Amounts mismatch"); }
        if deadline <= env.ledger().timestamp() { panic!("Deadline must be in future"); }

        if contract_type == C_REP_LEND {
            ...
            let loan_amount = net_value - down_payment;
            let requested_ltv = (loan_amount * 100) / net_value;
            if requested_ltv > max_ltv {
                panic!("Credit Rejected: Requested LTV exceeds allowed tier limit");
            }
        } else {
            if down_payment != net_value {
        ...
        let mut commitment = Commitment {
            id, creator: creator.clone(), counterparty: counterparty.clone(),
            original_value, net_value, down_payment, first_release_amount, second_release_amount,
            contract_type: contract_type.clone(), status: S_PENDING, deadline,
            ...
            legal_doc_hash: legal_doc_hash.clone(), extra_data: Map::new(&env),
        ...
        ContractCreated { id, creator, contract_type, net_value, down_payment }.publish(&env);
        id
    }
    ...
    pub fn accept_commitment(env: Env, id: u64, funder: Address, verified_doc_hash: String) {
        ...
        ContractAccepted { id, counterparty: c.counterparty, escrowed_amount: c.down_payment }.publish(&env);
    }
    ...
    pub fn cancel_commitment(env: Env, id: u64, caller: Address, reason: Symbol) {
        ...
        Self::save_commitment(&env, id, &c);
        ContractCancelled { id, by: caller, refunded }.publish(&env);
    }
    ...
    pub fn update_payment_status(env: Env, id: u64, payment_status: Symbol) {
        ...
        Self::save_commitment(&env, id, &c);
        PaymentUpdated { id, status: payment_status }.publish(&env);
    }
    ...
    pub fn confirm_delivery(env: Env, id: u64, counterparty: Address, biometric_hash: String) {
        ...
        c.extra_data.set(String::from_str(&env, "delivery_proof_hash"), biometric_hash.clone());

        Self::save_commitment(&env, id, &c);
        DeliveryConfirmed { id, amount: c.first_release_amount, proof_hash: biometric_hash }.publish(&env);
    }
    ...
    pub fn confirm_review(env: Env, id: u64, counterparty: Address) {
        ...
        CertificateRegistryClient::new(&env, &cert_registry_addr).issue_certificate(
            &env.current_contract_address(), &cert_id, &c.creator, &c.counterparty,
            &String::from_str(&env, "Mithaq Verified")
        );
        ...
        ContractCompleted { id, final_payout: payout, fees_paid }.publish(&env);
    }
    ...
    pub fn auto_release(env: Env, id: u64) {
        ...
        Self::save_commitment(&env, id, &c);
        AutoReleased { id, payout }.publish(&env);
    }
    ...
    pub fn open_dispute(env: Env, id: u64, caller: Address, reason: String) -> u64 {
        ...
        DisputeOpened { dispute_id, contract_id: id, by: caller, reason }.publish(&env);
        dispute_id
    }
    ...
    pub fn resolve_dispute(env: Env, dispute_id: u64, winner: Address, verdict_text: String, penalty_type: Symbol) {
        ...
        DisputeResolved { dispute_id, winner, verdict: verdict_text, payout }.publish(&env);
    }
    ...
    pub fn release_milestone(env: Env, id: u64, caller: Address, milestone: u32) {
        ...
        let current_milestone = c.extra_data.get(String::from_str(&env, "milestone")).unwrap_or(0u32);
        if milestone <= current_milestone { panic!("Double spending/release attempt"); }

        c.extra_data.set(String::from_str(&env, "milestone"), milestone);
        ...
        Self::save_commitment(&env, id, &c);
        MilestoneReleased { id, milestone, amount: amount_per_milestone }.publish(&env);
    }
    ...
    pub fn pay_installment(env: Env, id: u64, caller: Address, installment_num: u32) {
        ...
        let current_installment = c.extra_data.get(String::from_str(&env, "installment")).unwrap_or(0u32);
        if installment_num <= current_installment { panic!("Installment previously cleared"); }

        c.extra_data.set(String::from_str(&env, "installment"), installment_num);
        ...
        Self::save_commitment(&env, id, &c);
        InstallmentPaid { id, installment: installment_num, amount: amount_per_installment }.publish(&env);
    }
    ...
    pub fn get_admin(env: Env) -> Address { env.storage().persistent().get(&DataKey::Admin).expect("Admin not set") }

    /// الحصول على نسب توزيع المصادرات الحالية
