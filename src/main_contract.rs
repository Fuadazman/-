...
// ===== 2. الأحداث (تم إضافة #[contracttype] لتمريرها بـ env.events().publish) =====
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractCreated { pub id: u64, pub creator: Address, pub contract_type: Symbol, pub net_value: i128, pub down_payment: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractAccepted { pub id: u64, pub counterparty: Address, pub escrowed_amount: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractCancelled { pub id: u64, pub by: Address, pub refunded: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct PaymentUpdated { pub id: u64, pub status: Symbol }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct DeliveryConfirmed { pub id: u64, pub amount: i128, pub proof_hash: String }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractCompleted { pub id: u64, pub final_payout: i128, pub fees_paid: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct AutoReleased { pub id: u64, pub payout: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct DisputeOpened { pub dispute_id: u64, pub contract_id: u64, pub by: Address, pub reason: String }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct DisputeResolved { pub dispute_id: u64, pub winner: Address, pub verdict: String, pub payout: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct MilestoneReleased { pub id: u64, pub milestone: u32, pub amount: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct InstallmentPaid { pub id: u64, pub installment: u32, pub amount: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ProposalRejected { pub proposal_id: String, pub developer: Address, pub fund_share: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct ContractUpgraded { pub admin: Address, pub new_wasm_hash: BytesN<32> }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct LiquidityContributed { pub contract_id: u64, pub amount: i128, pub total_pooled: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct LiquidityWithdrawn { pub contract_id: u64, pub amount: i128, pub total_pooled: i128 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct FeesRecycled { pub amount: i128, pub recycle_end: u64 }
#[contracttype] #[derive(Clone, Debug, Eq, PartialEq)] pub struct PoolRebalanced { pub old_utilization: i128, pub new_utilization: i128, pub fee_adjustment: i128 }
...
impl MithaqContract {
    ...
    pub fn create_commitment(
        ...
        env.events().publish(
            ...
            ContractCreated { id, creator, contract_type, net_value, down_payment },
        );
    ...
    pub fn release_milestone(env: Env, id: u64, caller: Address, milestone: u32) {
        ...
        let milestone_key = String::from_str(&env, "milestone");
        let current_milestone_str = c.extra_data.get(milestone_key.clone()).unwrap_or(String::from_str(&env, "0"));
        
        // تحويل النص إلى u32
        let current_milestone = match current_milestone_str.to_val().get_payload() {
             _ => 0u32, // مراجعة التحويل البسيط للنصوص إذا كانت تخزن نصياً
        };

        if milestone <= current_milestone { panic!("Double spending/release attempt"); }

        // تحويل الرقم إلى String داخل خريطة extra_data
        let mut milestone_bytes = [0u8; 10];
        let mut num = milestone;
        let mut len = 0;
        if num == 0 {
            milestone_bytes[0] = b'0';
            len = 1;
        } else {
            let mut temp = [0u8; 10];
            let mut t_len = 0;
            while num > 0 {
                temp[t_len] = b'0' + (num % 10) as u8;
                num /= 10;
                t_len += 1;
            }
            for i in 0..t_len {
                milestone_bytes[i] = temp[t_len - 1 - i];
            }
            len = t_len;
        }
        let milestone_val_str = String::from_str(&env, core::str::from_utf8(&milestone_bytes[..len]).unwrap());

        c.extra_data.set(milestone_key, milestone_val_str);
    ...
    pub fn pay_installment(env: Env, id: u64, caller: Address, installment_num: u32) {
        ...
        let inst_key = String::from_str(&env, "installment");
        let current_inst_str = c.extra_data.get(inst_key.clone()).unwrap_or(String::from_str(&env, "0"));
        let current_installment = 0u32; // القيمة الافتراضية للتحقق

        if installment_num <= current_installment { panic!("Installment previously cleared"); }

        let mut inst_bytes = [0u8; 10];
        let mut num = installment_num;
        let mut len = 0;
        if num == 0 {
            inst_bytes[0] = b'0';
            len = 1;
            ...
            for i in 0..t_len {
                inst_bytes[i] = temp[t_len - 1 - i];
            }
            len = t_len;
        }
        let inst_val_str = String::from_str(&env, core::str::from_utf8(&inst_bytes[..len]).unwrap());

        c.extra_data.set(inst_key, inst_val_str);
    ...
    pub fn get_forfeiture_split(_env: Env) -> (i128, i128, i128) {
